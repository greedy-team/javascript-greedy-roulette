...
- 현재 자금은 `10,000`이 표시된다.
- 현재 라운드는 `0`이 표시된다.
- 결과 영역은 보이지 않는다.
- 다시 시작 버튼은 보이지 않는다.
- 베팅 버튼과 중단 버튼은 활성화 상태다.

---

#### 2) 베팅 시작 직후(베팅 버튼 클릭 직후)

- 현재 자금은 베팅 금액만큼 즉시 차감되어 표시된다.
- 베팅 버튼과 중단 버튼은 비활성화된다.
- 결과 영역이 표시된다.
- 결과 영역에 아래 문구가 표시된다.
...
- 결과 색상은 시각적으로 구분되어 표시된다.
- 베팅 버튼과 중단 버튼은 다시 활성화된다.
...
- 아래 조건 중 하나를 만족하면 게임이 종료된다.
  - 사용자가 중단 버튼을 클릭한 경우
  - 자금이 0원 이하가 된 경우 (파산)
    - 룰렛 결과와 함께 결과 영역에 `게임이 곧 종료됩니다.` 파산 안내 메시지가 표시된다.
    - 2초 뒤에 게임 종료 화면으로 전환된다.
- 종료 시 결과 영역에 최종 결과가 표시된다.
...
- 베팅 입력 영역(색상 선택, 금액 입력, 버튼)은 숨김 처리된다.
- 다시 시작 버튼이 표시된다.
